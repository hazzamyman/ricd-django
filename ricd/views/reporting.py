...
def monthly_report_form(request, council_id, period):
    logger.info(f"Accessing monthly report form for council {council_id}, period {period}")
    ...
    if request.method == 'POST':
        logger.info(f"Saving monthly report for council {council_id}, period {period}")
...
def quarterly_report_form(request, council_id, period):
    logger.info(f"Accessing quarterly report form for council {council_id}, period {period}")
    is_authenticated = request.user.is_authenticated
    is_ricd = is_authenticated and (getattr(request.user, 'user_type', '') == 'ricd' or request.user.is_staff)
    council = get_object_or_404(Council, id=council_id)
    try:
    ...
    if request.method == 'POST':
        logger.info(f"Saving quarterly report for council {council_id}, period {period}")
...
def ricd_review_report(request, report_id):
    logger.info(f"Accessing RICD review report {report_id}")
    ...
    if request.method == 'POST':
        logger.info(f"Saving RICD review for report {report_id}, status: {request.POST.get('ricd_status')}")
