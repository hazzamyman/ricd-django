...
def funding_schedule_create(request, project_id):
    """Create a new funding schedule for a dwelling project"""
    ...
    if request.method == 'POST':
        ...
        FundingSchedule.objects.create(
            project=project,
            project_type=FundingSchedule.ProjectType.DWELLING,
            amount=amount,
...
@login_required
def funding_schedule_create_land(request, land_project_id):
    """Create a new funding schedule for a land/infra project"""
    ...
    if request.method == 'POST':
        ...
        FundingSchedule.objects.create(
            ...
            project_type=FundingSchedule.ProjectType.LAND,
            amount=amount,
            contingency=contingency,
            payment_split=payment_split
        )
...
def funding_schedule_delete(request, fs_id):
    ...
    if fs.project_type == FundingSchedule.ProjectType.DWELLING and fs.project:
        project_id = fs.project.id
        redirect_url = 'projects:project_detail'
    elif fs.land_project:
        project_id = fs.land_project.id
