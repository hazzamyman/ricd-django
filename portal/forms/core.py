...
from ricd.models import Council, Program, Officer, SiteConfiguration, Defect, ProjectReportConfiguration
from ricd.models.work import Work
...
class DefectForm(forms.ModelForm):
    ...
    def __init__(self, user=None, *args, **kwargs):
        ...
        if user:
            ...
            if user_council:
                # Council user - filter works to their council's projects
                self.fields['work'].queryset = Work.objects.filter(
                    address__project__council=user_council
                ).select_related('address__project')
            else:
                # RICD user - no filtering
                self.fields['work'].queryset = Work.objects.all().select_related('address__project')
...
# Project Report Configuration Form
class ProjectReportConfigurationForm(forms.ModelForm):
    """Form for project report configuration"""

    class Meta:
        model = ProjectReportConfiguration
        fields = '__all__'
        widgets = {
            'monthly_tracker_groups': forms.SelectMultiple(attrs={'class': 'form-select'}),
            'quarterly_report_groups': forms.SelectMultiple(attrs={'class': 'form-select'}),
            'stage1_step_groups': forms.SelectMultiple(attrs={'class': 'form-select'}),
            'stage2_step_groups': forms.SelectMultiple(attrs={'class': 'form-select'}),
        }